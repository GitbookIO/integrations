...
export async function* streamCopilotResponse(
    ...
    const stream = api.orgs.streamAskInOrganization(
        gitbookInstallation.target.organization,
        {
            query: query,
            format: 'markdown',
        }
    ...
    for await (const event of stream) {
        if (event.type === 'answer' && event.answer && event.answer.answer && 'markdown' in event.answer.answer) {
    ...
    if (pageSources.length > 0) {
        try {
            yield createReferencesEvent(
                await Promise.all(pageSources
                .map(async source => {
                    const { data: page } = await api.spaces.getPageInRevisionById(source.space, source.revision, source.page);
