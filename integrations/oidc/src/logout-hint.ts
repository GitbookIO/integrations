...
/**
 * Path the logout hint cookie is scoped to: the visitor authentication endpoint,
 * which is where it is read back.
 */
export function getLogoutHintCookiePath(scope: LogoutHintScope): string {
  return `/v1/integrations/${scope.integration}/installations/${scope.installation}/sites/${scope.site}/visitor-auth`;
}
...
function serializeScope(scope: LogoutHintScope): string {
  return `${scope.integration}:${scope.installation}:${scope.site}`;
}
