...
type GitlabConfigureStepId = 'token' | 'project' | 'monorepo' | 'priority';

type GitlabConfigureStepAction = { action: 'step.go'; step: GitlabConfigureStepId };

export type GitlabConfigureAction =
    | (
          | { action: 'save.hello'; hello: string }
...
export type GitlabConfigureState = Omit<SpaceInstallationConfiguration, 'project'> & {
    ...
    activeStepId: 'token' | 'project' | 'branch' | 'configuration';
};
